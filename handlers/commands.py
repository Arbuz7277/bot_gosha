...
def setup_handlers(bot):
    ...
    def callback(call):
        ...
        if action == "pay_accept":
            ...
            try:
                if hide_check1:
                    bot.send_message(user_id, check2, parse_mode='HTML')
            except:
                text += "❌ Ошибка отправки чека вам.\n"

            try:
                if hide_check2:
                    bot.send_message(reply_id, check1, parse_mode='HTML')
            except:
            ...
            bot.edit_message_text(f'<a href="https://telegra.ph/Komandy-Goshi-11-02">Команды</a>', call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=markup, disable_web_page_preview=True)
            ...
            def delayed_message(chat_id, bid, u1, u2, user_id, rid, users):
                ...
                win = round(bid - (bid / 100 * COMMISION_DICE), 2)
                ...
                if dice1 > dice2:
                    ...
                    u1['money'] += win
                    u2['money'] -= bid
                    databank['money'] += round((bid / 100 * COMMISION_DICE), 2)
    ...
    def cmd_help(message):
        bot.reply_to(message, f'<a href="https://telegra.ph/Komandy-Goshi-11-02">Команды</a>', parse_mode='HTML', disable_web_page_preview=True)
    ...
    def cmd_farm(message):
        ...
        if databank['money'] < 50:
            bot.reply_to(message, 'Not enough money in the bank')
            return
    ...
    def cmd_casino(message):
        ...
        if p == 1:
            ...
            bot.reply_to(message, f"✅ <b>Успех!</b>\n\nВы выиграли {win} {get_coin_form(win)}! (+{round(win - bid, 2)}, {multi}x)", parse_mode='HTML')
            ...
            bot.reply_to(message, f"❌ <b>Неудача!</b>\n\nВы получили {loser} {get_coin_form(bid)}! ({round(loser - bid, 2)}, {multi}x)", parse_mode='HTML')
    ...
    def cmd_number(message):
        ...
        if args[1] == 'kd':
            bot.reply_to(message, f'⏳ Cooldown (KD): {round(other_data['number_money'] / 30 + KD_NUMBER, 2)} секунд.')
            return
    ...
    def text(message):
        ...
        if message and message.content_type == 'text':
            ...
            elif args[0] in ['.казино', 'казик', 'деп', 'депнуть', 'casino'] and len(args) >= 2:
                if bot_stat(message, bot): return
                user_id = message.from_user.id
                add_chat(message.chat.id)
                if str(user_id) not in users:
